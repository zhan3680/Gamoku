...
def detect_rowC(board, col, y, x, length, d_y, d_x):
    ...
    while j < ranging:
        ...
        if col_length==length:
           if d_y==1 and d_x==-1:
               if is_bounded(board,y+j-col_length+(col_length-1)*d_y, x-j+col_length+(col_length-1)*d_x, length, d_y, d_x)=="OPEN":
                  open_seq_count+=1
                  col_length=0
               elif is_bounded(board,y+j-col_length+(col_length-1)*d_y, x-j+col_length+(col_length-1)*d_x, length, d_y, d_x)=="SEMIOPEN":
                  semi_open_seq_count+=1
                  col_length=0
               else:
                  closed_seq_count+=1
